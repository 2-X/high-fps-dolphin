...
This Mac (Apple Silicon M2 Max) sustains only ~2x emulation for Sunshine → **120fps is
...
- **The stock fpspatch Gecko bundle does essentially NOTHING under BSE** — any write to
...
| 13 | Noki pollution perf | Ep.1 caps ~105fps; goop vanishes/freezes Bianco | Per-frame degree-count EFB readbacks 4x too often | `noki_gate` C2 @`0x8019D8C8` (call-site gates 1-in-2G) + `noki_copy_gate` @`0x802F8CF8` (+ retired dedupe) | guarded port CRASHES Bianco Ep.1 under BSE (A/B-confirmed 2026-08-13) — DISABLED. Static analysis exonerates the code (no kxe hook collision, regs safe, byte-faithful port); top remaining suspect = Kuribo runtime vs low-arena counters `0x16E0/0x16E4`; need a live scratch dump + one panic PC | FIXED (stock); BROKEN/DISABLED (BSE) |
...
**Shipped fix:** `PROXIMITY_GLOW` (a distance-reimplemented proximity glow, ~354u radius,
XZ distance) + `FORCEOPEN` (C2 @`0x801EB034`+0x7 that calls the real `startOpen`). Both are
rate-independent. `--no-forceopen` respects story locks. User: "works amazingly, stunned."
...
enabled.** Under BSE, the stock FOV must be DISABLED (its caller-whitelist breaks + causes
...
pitches up 2x). Also `AudioBufferSize=136` (default 80 underran at level-load → looped a
...
cadence vs emulated time → heuristic chronically misfires → every sequenced BGM note killed
at birth. **Fix:** `0440CDB4 00000000` (`DSP_LIMIT_RATIO=0` @`0x8040CDB4`). Plus the tempo
guard `BGM_TEMPO_GUARD` C2 @`0x8031B8C8` (substitute 1.0 for a 0.0 outer tempo proportion
across scene transitions). Both rate-independent, vanilla addrs → should work under BSE.
...
Fly→Explosion at flyTimer ~8–11 EVERY time regardless of velocity → **time-based trigger**
...
→ goop vanishes. v1/v2 blr'd the WHOLE perform (skipped the drain → batched stamps → the
M-portal ripple-lateness AND the Bianco J3D self-loop freeze that OOM'd Metal to 64GB); v3
...
anim reaches the `mFadeAnim==0.0` deletion tick → emitter left alive+emitting forever, one
leaked per pause. **`$StarFix v4`** = v2 (`pauseOut` @`0x8014A850` stops con+0x124 + pause
emitter) + v3 (@`0x80155D8C` stops the previous bounce emitter before each re-create) +
watchdog (@`0x80324EB8` in `JPAEmitterManager::calcBase`: if mgr==`0x8040E1E4` &&
maxFrame==0 && !stopped && age>600 → STOP_EMIT, catches the 3 stranded banner emitters).
NEEDS-TEST under BSE (safe once particles are confirmed 60Hz).
...
hook tests counter+1 without storing (it runs first). **NEVER gate SE at call sites** — the
...
a bare 04 is safe. Enable v3 OR v2, never both.
...
did not restore it; it is its own PENDING investigation.
...
(hook @`0x80014DA8`) processes SE requests at render rate, which outruns the 120 Hz substep
request rate → thrashes the 64-voice pool → steal-kills every BGM note at birth. Gate to
...
and never re-arms. The 30 Hz audio pump alone does not tame it → a dedicated gate keyed on a
...
rate stores that bypass the getter). NOT nerve/spine timers (those are substep-invariant —
a global mTime÷G hook would REGRESS ~200 correct timers, do NOT build it). **`anmrate()`**
...
Online-only: entering controller settings while idle threw an ISI exception. Root cause: the
ghost bot's unbounded `AnimId` ramp indexed past `gMarioAnimeData[411]` in
...
User suspected fast plaza gondolas/boats. Verdict: `TFruitsBoat` one mode is stock speed,
...
readback) and breaks the flare (draws through geometry). Kept for reference, off by default.
...
parity addrs; Game-clock v15 + Petey v16 re-enabled; FOV disabled; the idle→settings crash
FIXED (anim clamp).
...
FPS_30/aspect-0 every launch, nothing persisted; the prior session ended with the working
old-ISO+bridge setup torn down for fork-ISO testing). New tooling: `set_bse_fps.py`
(no-stage RAM poke, fps+aspect before scene-init) + `play120.sh` (one-command launcher,
`--ghost` for the full online stack). bridge.py pokes are now discovery-validated (no blind
writes; `--fps` arg). `$FOV 60` disabled under BSE (shimmer-only mismatch seen live).
Bianco Ep.1 crash A/B-isolated to the BSE Noki gate → DISABLED (item 13; code statically
exonerated, suspect low-arena scratch vs Kuribo runtime). Companion v2 adds BSE-guarded/
self-gated: clock v15 (10), anmrate (27, covers Petey 11 + Gooper), animal ×4 (19), Poink
v14 (12) — all installed+enabled, NEEDS in-game test: birds, Petey fight, Poink balloons,
Gooper Blooper, race timers.
