...
- How 180fps works (condensed; full derivation in HANDOFF-INPUT-BUG §2/§11):
  ...
  - Perform-list cues (decomp `include/JSystem/JDrama/JDRViewObj.hpp`):
    ...
    **CUE_CALC_ANIM=0x2 fires on the LAST substep of each rendered frame only**
    (29.97Hz stock → 119.88Hz v12, 4× more often), which is why v12 forces
...
3. **Flight** (`flyBehavior` per spine tick): `++flyTimer > 300` → Explosion nerve;
   ...
   so flight is perfectly straight. Velocity integrated by `TLiveActor::moveObject`
...
a tick-count invariant. Every rate-scaled quantity checked (see §3.4). The defect must
...
## 7. Traps (inherited from the project, all verified real)
